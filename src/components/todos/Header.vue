<template>
    <header
        class="md:h-[var(--header-height-desktop)] bg-white dark:bg-gray-800/60 border-b border-gray-200 dark:border-gray-600 py-4 px-5 mb-4 xl:mb-[var(--header-margin-bottom-desktop)]">
        <!-- Add modal -->
        ...
        <div class="flex">
            <div
                ...
                <button @click="openTaskAddModal"
                    class="relative inline-flex items-center max-w-max justify-center p-0.5 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group dark:text-white primary-button-bg"
                    :aria-label="$t('add_task')">
                    <span
                        class="flex items-center gap-1 relative px-3 py-1.5 md:py-2 md:px-4 transition-all ease-in duration-75 bg-white dark:bg-gray-900 rounded-md group-hover:bg-opacity-0">
                        <AddIcon size="19" />
                ...
                <!-- Search bar -->
                <form class="max-w-sm w-full" @submit.prevent>
            ...
            $t("search") }}</label>
                    <div class="relative">
                        ...
                        <button type="submit"
                            class="absolute end-1 bottom-1 bg-gray-200 dark:bg-gray-500 dark:text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-1.5 hover:brightness-95 transition-[filter] duration-150">
                            {{ $t("search") }}
                        </button>
                    </div>
                </form>
            ...
            <div class="flex items-center gap-2 lg:gap-3 justify-self-end">
                ...
                <span>
                    <button v-if="isCurrentThemeDark" :aria-labelledby="$t('button_titles.light_theme')"
                        :title="$t('button_titles.light_theme')" type="button" @click="() => switchTheme('light')"
                        class="hs-dark-mode font-medium text-gray-800 dark:text-lime-500 rounded-full dark:hover:bg-lime-50 hover:bg-gray-200 focus:outline-none focus:bg-gray-200"
                    ...
                    <button v-else type="button" :aria-labelledby="$t('button_titles.dark_theme')"
                        :title="$t('button_titles.dark_theme')" @click="() => switchTheme('dark')"
                        class="block hs-dark-mode font-medium text-gray-800 dark:text-lime-500 rounded-full dark:hover:bg-lime-50 hover:bg-gray-200 focus:outline-none focus:bg-gray-200"
                        data-hs-theme-click-value="dark">
                        <span class="group inline-flex shrink-0 justify-center items-center size-9">
                ...
                <!-- Language switcher -->
                <button @click="toggleLanguage" :aria-labelledby="languageButtonTitle" :title="languageButtonTitle"
                    class="hs-dark-mode font-medium text-gray-800 dark:text-lime-500 dark:hover:bg-lime-50 rounded-full hover:bg-gray-200 focus:outline-none focus:bg-gray-200">
                    <span class="group inline-flex shrink-0 justify-center items-center size-9">
                        <svg class="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="35" height="35"
                            viewBox="0 0 24 24">
...
const languageButtonTitle = computed(() => (currentLanguage.value === 'en' ? 'التغيير اللغة اي العربية' : 'Change language to Arabic'));
