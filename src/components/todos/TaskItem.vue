<template>
    <div
        v-if="props.item"
        :id="props.item.id"
        :class="`${isDesktop ? 'handle cursor-grab' : ''} flex flex-col p-3 group sm:min-h-[7.8rem] select-none rounded-[0.5rem] mb-[0.25rem] w-full border-[1px] relative border-light-taskBorder dark:border-dark-taskBorder shadow-[0_.0625rem_.125rem_rgba(0,0,0,.055)] hover:shadow-[0_.0625rem_.1875rem_rgba(0,0,0,.106),0_.0625rem_.125rem_-.0625rem_rgba(0,0,0,.106)] bg-light-white dark:bg-gray-600 min-h-[2.5rem]`.trim()">
        <!-- task options -->
        <div class="md:absolute p-1 group-hover:opacity-100 group-hover:pointer-events-auto opacity-100 pointer-events-auto sm:opacity-0 sm:pointer-events-none transition-opacity duration-100 flex items-center top-1.5 z-[4] max-h-[2rem] rtl:left-2 right-2 rtl:right-auto rounded-[0.375rem] mb-[0.25rem] w-max border-[1px] border-light-taskBorder dark:border-dark-taskBorder shadow-[0_.0625rem_.1875rem_rgba(0,0,0,.106),0_.0625rem_.125rem_-.0625rem_rgba(0,0,0,.106)] bg-light-white dark:bg-gray-600">
            ...
            <button v-if="!isDesktop" :tooltipTitle="$t('move')" class="md:hidden cursor-grab handle p-1 tooltip hover:bg-light-hover dark:hover:bg-dark-hover rounded-[0.375rem]" aria-label="move item" ><MoveIcon class="dark:text-gray-50"/></button>
        </div>
        <!-- task title -->
        ...
        <!-- task description -->
        <p class="line-clamp-2 whitespace-normal text-light-text dark:text-dark-text text-sm mb-2" :title="props.item[props.descriptionKey]">{{ props.item[props.descriptionKey] }}</p>
        <!-- priority & due date -->
        <div class="flex justify-end gap-2 mt-auto self-end">
            ...
            <span class="inline-flex gap-1 items-center border border-light-taskBorder dark:border-dark-taskBorder rounded-md py-0.5 px-1">
                ...
                <span class="text-xs capitalize dark:text-gray-300" :title="props.item.priority_level">{{ $t(`priorities.${props.item.priority_level}`) }}</span>
            </span>
        </div>
    </div>
</template>
...
<script setup>
    ...
    import { EditIcon, ViewIcon, DeleteIcon, MoveIcon } from "@/components/generic";
    ...
    onMounted(() => {
        ...
        // Note: we did not use the CSS's media queries
        // because the "handle cursor-grab" should be included as classes to work properly with the "vue-draggable-next" package
